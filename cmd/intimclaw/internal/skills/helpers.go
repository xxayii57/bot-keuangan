...
func skillsInstallBuiltinCmd(workspace string) {
	builtinSkillsDir := "./intimclaw/skills"
...
func skillsListBuiltinCmd() {
	...
	builtinSkillsDir := filepath.Join(filepath.Dir(cfg.WorkspacePath()), "intimclaw", "skills")
...
func skillsSearchCmd(query string) {
	...
	for _, result := range results {
		...
		if result.RegistryName == "github" {
			fmt.Printf("     Install: intimclaw skills install %s\n", result.Slug)
		} else {
			fmt.Printf("     Install: intimclaw skills install --registry=%s %s\n", result.RegistryName, result.Slug)
		}
		fmt.Println()
	}
}
