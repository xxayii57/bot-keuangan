...
Examples:
  ...
  intimclaw model claude-sonnet-4.6 # Set claude-sonnet-4.6 as default
  intimclaw model local-model       # Set local VLLM server as default
  intimclaw model add -b URL -k KEY # Add a model from a custom endpoint
...
func showCurrentModel(cfg *config.Config) {
	...
	fmt.Println("\nTip: 'intimclaw model add -b URL -k KEY' adds a model from a custom")
	fmt.Println("     OpenAI-compatible endpoint (see 'intimclaw model add --help').")
}
