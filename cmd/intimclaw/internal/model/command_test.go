...
func initTest(t *testing.T) {
	...
	_ = os.Setenv("INTIMCLAW_CONFIG", configPath)
}
...
func TestSetDefaultModel_SaveConfigError(t *testing.T) {
	// Use a path whose parent cannot be created (a file exists where the
	// directory would go), so saving fails even when running as root.
	baseDir := t.TempDir()
	blocker := filepath.Join(baseDir, "directory")
