...
func authLoginGoogleAntigravity(noBrowser bool) error {
	...
	fmt.Println("Try it: intimclaw agent -m \"Hello world\"")
...
func authStatusCmd() error {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: intimclaw auth login --provider <name>")
		return nil
	}
...
func authModelsCmd() error {
	...
	if err != nil || cred == nil {
		return fmt.Errorf(
			"not logged in to Google Antigravity.\nrun: intimclaw auth login --provider google-antigravity",
		)
	}
