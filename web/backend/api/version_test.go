...
func setupVersionTestIsolation(t *testing.T) {
	...
	originalFinder := FindIntimClawBinaryForInfo
	originalRunner := runIntimClawVersionOutput
	originalFallback := launcherBuildInfoForVersion
	t.Cleanup(func() {
		currentGatewayVersionState = originalGatewayState
		FindIntimClawBinaryForInfo = originalFinder
		runIntimClawVersionOutput = originalRunner
...
func TestGetSystemVersionUsesIntimClawBinaryInfo(t *testing.T) {
	...
	FindIntimClawBinaryForInfo = func() string { return "intimclaw" }
	runIntimClawVersionOutput = func(_ context.Context, _ string) (string, error) {
		return "🦞 intimclaw v1.2.3 (git: deadbeef)\n  Build: 2026-03-27T12:34:56Z\n  Go: go1.25.8\n", nil
	}
...
func TestParseIntimClawVersionOutput(t *testing.T) {
	setupVersionTestIsolation(t)

	raw := "\u001b[1;31m████\u001b[0m\n🦞 intimclaw 18ec263 (git: 18ec2631)\n  Build: 2026-03-27T10:43:34+0000\n  Go: go1.25.8\n"
	got, ok := parseIntimClawVersionOutput(raw)
	if !ok {
		t.Fatal("parseIntimClawVersionOutput() should parse valid output")
	}
...
func TestParseIntimClawVersionOutputIgnoresUsageLine(t *testing.T) {
	setupVersionTestIsolation(t)

	raw := "Usage: intimclaw version [flags]\n"
	got, ok := parseIntimClawVersionOutput(raw)
	if ok {
		t.Fatalf("parseIntimClawVersionOutput() parsed usage line unexpectedly: %#v", got)
	}
}

func TestParseIntimClawVersionOutputAcceptsLetterOnlyHashVersion(t *testing.T) {
	setupVersionTestIsolation(t)

	raw := "intimclaw abcdefa (git: abcdefabcdefabcdefabcdefabcdefabcdefabcd)\n"
	got, ok := parseIntimClawVersionOutput(raw)
	if !ok {
		t.Fatal("parseIntimClawVersionOutput() should parse letter-only hash version")
	}
...
func TestResolveSystemVersionInfoCachesWhileGatewayAlive(t *testing.T) {
	...
	runIntimClawVersionOutput = func(_ context.Context, _ string) (string, error) {
		runCount++
		return fmt.Sprintf("intimclaw v1.2.%d\n", runCount), nil
	}
...
func TestResolveSystemVersionInfoInvalidatesCacheWhenGatewayStops(t *testing.T) {
	...
	runIntimClawVersionOutput = func(_ context.Context, _ string) (string, error) {
		runCount++
		return fmt.Sprintf("intimclaw v2.0.%d\n", runCount), nil
	}
...
func TestResolveGatewayBinaryForVersionInfoPrefersGatewayCommandPath(t *testing.T) {
	...
	gateway.cmd = &exec.Cmd{Path: "/tmp/intimclaw-from-gateway"}
	gateway.mu.Unlock()
	t.Cleanup(func() {
		gateway.mu.Lock()
	...
	if got != "/tmp/intimclaw-from-gateway" {
		t.Fatalf("exec path = %q, want %q", got, "/tmp/intimclaw-from-gateway")
	}
}
