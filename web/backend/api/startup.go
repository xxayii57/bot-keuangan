...
const (
	autoStartEntryName = "IntimClawLauncher"
	launchAgentLabel   = "io.intimclaw.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "intimclaw-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=IntimClaw Web",
			"Comment=Start IntimClaw Web on login",
