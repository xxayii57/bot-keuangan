...
var (
	...
	FindIntimClawBinaryForInfo   = resolveGatewayBinaryForVersionInfo
	runIntimClawVersionOutput    = executeIntimClawVersion
...
// resolveSystemVersionInfo prefers the actual intimclaw binary version output,
...
func (h *Handler) resolveSystemVersionInfoUncached(ctx context.Context) systemVersionResponse {
	...
	execPath := strings.TrimSpace(FindIntimClawBinaryForInfo())
	if execPath == "" {
		return fallback
	}
	...
	output, err := runIntimClawVersionOutput(cmdCtx, execPath)
	if err != nil {
		return fallback
	}

	parsed, ok := parseIntimClawVersionOutput(output)
	if !ok {
		return fallback
	}
...
func resolveGatewayBinaryForVersionInfo() string {
	...
	return utils.FindIntimClawBinary()
}
...
// executeIntimClawVersion runs the version subcommand against the
// discovered intimclaw executable.
func executeIntimClawVersion(ctx context.Context, execPath string) (string, error) {
...
// parseIntimClawVersionOutput extracts version/build/go fields from CLI output.
// It accepts banner/ANSI-decorated output and only requires the version line.
func parseIntimClawVersionOutput(raw string) (systemVersionResponse, bool) {
