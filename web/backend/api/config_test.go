...
func setupPicoEnabledEnv(t *testing.T) (string, func()) {
	...
	oldPicoHome := os.Getenv("INTIMCLAW_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("INTIMCLAW_HOME", filepath.Join(tmp, ".intimclaw")); err != nil {
		t.Fatalf("set INTIMCLAW_HOME: %v", err)
	}
	...
	cleanup := func() {
		...
		if oldPicoHome == "" {
			_ = os.Unsetenv("INTIMCLAW_HOME")
		} else {
			_ = os.Setenv("INTIMCLAW_HOME", oldPicoHome)
		}
	}
