...
var gateway = struct {
	...
	logs                *LogBuffer
	pidData             *ppid.PidFileData // pid file data read from intimclaw.pid.json
...
// isLikelyGatewayProcess returns whether PID appears to be a intimclaw gateway
...
func isLikelyGatewayProcess(pid int) (bool, bool) {
	...
	if runtime.GOOS == "windows" {
		...
		if strings.HasPrefix(line, "\"") {
			if strings.Contains(line, "\"intimclaw.exe\"") {
				return true, true
			}
			return false, true
		}
...
// represents "intimclaw gateway ..." regardless of executable filename.
...
func (h *Handler) validateGatewayPidData(
	...
	if gatewayProcess, inspected := gatewayProcessMatcher(pidData.PID); inspected {
		if !gatewayProcess {
			return false, true, "pid process command is not intimclaw gateway"
		}
		return true, true, ""
	}
...
func (h *Handler) startGatewayLocked(initialStatus string, existingPid int) (int, error) {
	...
	// Start new process
	// Locate the intimclaw executable
	execPath := utils.FindIntimClawBinary()
	...
	pid = cmd.Process.Pid
	logger.InfoC("gateway", fmt.Sprintf("Started intimclaw gateway (PID: %d) from %s", pid, execPath))
...
// handleGatewayStart starts the intimclaw gateway subprocess.
//
