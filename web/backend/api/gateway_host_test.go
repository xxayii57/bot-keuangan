...
func TestBuildWsURLDoesNotTrustOriginWhenProxyOmitsForwardedProto(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "http://launcher.local/api/pico/info", nil)
	req.Host = "test-host.intimclaw.lan"
	req.Header.Set("Origin", "https://test-host.intimclaw.lan")

	if got := h.buildWsURL(req); got != "ws://test-host.intimclaw.lan:80/pico/ws" {
		t.Fatalf(
			"buildWsURL() = %q, want %q",
			got,
			"ws://test-host.intimclaw.lan:80/pico/ws",
		)
	}
}
