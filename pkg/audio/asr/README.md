...
This package handles speech-to-text for IntimClaw voice input.
...
IntimClaw does not keep ASR API keys inside the `voice` section.
...
This is the recommended pattern because it is explicit, reusable, and consistent with the rest of IntimClaw's model configuration.
...
- You can omit `api_base` and IntimClaw will use Groq's default API base automatically.
...
IntimClaw currently supports three main ASR routes:
...
## How IntimClaw Chooses a Transcriber
...
2. If that resolved model is:
   - an `elevenlabs` provider model, IntimClaw uses the ElevenLabs transcriber.
   - an OpenAI-compatible Whisper model, IntimClaw uses the Whisper transcriber.
   - an audio-capable chat model, IntimClaw uses `AudioModelTranscriber`.
3. **Fallback path**: if `voice.model_name` is not set, IntimClaw performs a compatibility scan through `model_list` for legacy auto-detected ASR entries.
