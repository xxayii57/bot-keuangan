...
func TestResolveInstanceRoot_UsesIntimClawHome(t *testing.T) {
	t.Setenv(config.EnvHome, "/custom/intimclaw/home")
	root, err := ResolveInstanceRoot()
	if err != nil {
		t.Fatalf("ResolveInstanceRoot() error = %v", err)
	}
	if root != "/custom/intimclaw/home" {
		t.Fatalf("ResolveInstanceRoot() = %q, want %q", root, "/custom/intimclaw/home")
	}
}
...
func TestBuildWindowsAccessRules(t *testing.T) {
	...
	for _, rule := range rules {
		if rule.Path == `C:\intimclaw` && rule.Mode == "rw" {
			foundRoot = true
		}
