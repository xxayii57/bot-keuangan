...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via INTIMCLAW_KEY_PASSPHRASE
...
//  2. INTIMCLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/intimclaw_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "INTIMCLAW_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading INTIMCLAW_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "INTIMCLAW_SSH_KEY_PATH"

// intimclawHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const intimclawHome = "INTIMCLAW_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "intimclaw-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (INTIMCLAW_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// INTIMCLAW_SSH_KEY_PATH env var or ~/.ssh/intimclaw_ed25519.key.
...
//   - exact match with INTIMCLAW_SSH_KEY_PATH env var
//   - within the INTIMCLAW_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with INTIMCLAW_SSH_KEY_PATH.
	...
	// Within INTIMCLAW_HOME.
	if picoHome := os.Getenv(intimclawHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="intimclaw-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set INTIMCLAW_SSH_KEY_PATH or place key at ~/.ssh/intimclaw_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (INTIMCLAW_SSH_KEY_PATH, INTIMCLAW_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. INTIMCLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/intimclaw_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the intimclaw-specific SSH key path if it exists.
