...
func TestResolve_EncKey_RoundTrip(t *testing.T) {
	dir := t.TempDir()
	sshKeyPath := filepath.Join(dir, "intimclaw_ed25519.key")
	...
	t.Setenv("INTIMCLAW_SSH_KEY_PATH", sshKeyPath)
	...
	t.Setenv("INTIMCLAW_KEY_PASSPHRASE", passphrase)
...
func TestResolve_EncKey_WithSSHKey(t *testing.T) {
	...
	// Set INTIMCLAW_SSH_KEY_PATH before Encrypt so the path passes allowedSSHKeyPath validation.
	t.Setenv("INTIMCLAW_KEY_PASSPHRASE", passphrase)
	t.Setenv("INTIMCLAW_SSH_KEY_PATH", sshKeyPath)
...
func TestResolve_EncKey_NoPassphrase(t *testing.T) {
	...
	_, err = r.Resolve(enc)
	if err == nil {
		t.Fatal("expected error when INTIMCLAW_KEY_PASSPHRASE is unset, got nil")
	}
}

func TestResolve_EncKey_BadCiphertext(t *testing.T) {
	t.Setenv("INTIMCLAW_KEY_PASSPHRASE", "some-passphrase")
...
func TestResolve_EncKey_WrongPassphrase(t *testing.T) {
	...
	t.Setenv("INTIMCLAW_KEY_PASSPHRASE", "wrong-passphrase")
...
func TestDeriveKey_SSHKeyNotFound(t *testing.T) {
	...
	t.Setenv("INTIMCLAW_KEY_PASSPHRASE", "passphrase")
	t.Setenv("INTIMCLAW_SSH_KEY_PATH", filepath.Join(dir, "nonexistent_key"))
...
// that are not under INTIMCLAW_SSH_KEY_PATH, INTIMCLAW_HOME, or ~/.ssh/.
