...
// with cf-mitigated: challenge triggers a retry using the honest intimclaw User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest intimclaw user agent
	if !strings.Contains(receivedUAs[1], "intimclaw") {
		t.Errorf("retry request UA = %q, want it to contain 'intimclaw'", receivedUAs[1])
	}
}
