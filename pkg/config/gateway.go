...
type GatewayConfig struct {
	Host      string `json:"host"                env:"INTIMCLAW_GATEWAY_HOST"`
	Port      int    `json:"port"                env:"INTIMCLAW_GATEWAY_PORT"`
	HotReload bool   `json:"hot_reload"          env:"INTIMCLAW_GATEWAY_HOT_RELOAD"`
	LogLevel  string `json:"log_level,omitempty" env:"INTIMCLAW_LOG_LEVEL"`
}
...
// The INTIMCLAW_LOG_LEVEL environment variable overrides the file value.
func ResolveGatewayLogLevel(path string) string {
	...
	if envLevel := os.Getenv("INTIMCLAW_LOG_LEVEL"); envLevel != "" {
