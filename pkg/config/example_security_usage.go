...
```bash
chmod 600 ~/.intimclaw/.security.yml
```
...
```bash
cp ~/.intimclaw/config.json ~/.intimclaw/config.json.backup
```

## Step 2: Create .security.yml
```bash
cp security.example.yml ~/.intimclaw/.security.yml
```

## Step 3: Fill in your API keys
Edit ~/.intimclaw/.security.yml and replace placeholders with your actual keys.

## Step 4: Simplify config.json (Recommended)
Remove sensitive fields from ~/.intimclaw/config.json:
...
```bash
rm ~/.intimclaw/config.json.backup
```
...
3. Set file permissions: chmod 600 ~/.intimclaw/.security.yml
...
```bash
# Channels
export INTIMCLAW_CHANNELS_TELEGRAM_TOKEN="token-from-env"
export INTIMCLAW_CHANNELS_DISCORD_TOKEN="discord-token-from-env"

# Web Tools
export INTIMCLAW_TOOLS_WEB_BRAVE_API_KEY="brave-key-from-env"
export INTIMCLAW_TOOLS_WEB_BAIDU_API_KEY="baidu-key-from-env"

# Skills
export INTIMCLAW_TOOLS_SKILLS_GITHUB_TOKEN="github-token-from-env"
```
...
- Verify the file permissions allow reading (chmod 600 ~/.intimclaw/.security.yml)
