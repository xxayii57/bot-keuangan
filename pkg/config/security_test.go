...
func TestSecurityPath(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "standard path",
			configDir: "/home/user/.intimclaw/config.json",
			want:      "/home/user/.intimclaw/.security.yml",
		},
		{
			name:      "nested path",
...
`
	...
	t.Run("test for env overwrite", func(t *testing.T) {
		...
		t.Setenv("INTIMCLAW_CHANNELS_QQ_APP_SECRET", "qq_app_secret_env")
		t.Setenv("INTIMCLAW_TOOLS_WEB_BRAVE_API_KEYS", "brave_key_env,abc")
