...
// INTIMCLAW_SSH_KEY_PATH to its path for the duration of the test. This is required
// whenever a test exercises encryption/decryption via credential.Encrypt or SaveConfig.
func mustSetupSSHKey(t *testing.T) {
	t.Helper()
	keyPath := filepath.Join(t.TempDir(), "intimclaw_ed25519.key")
	if err := credential.GenerateSSHKey(keyPath); err != nil {
		t.Fatalf("mustSetupSSHKey: %v", err)
	}
	t.Setenv("INTIMCLAW_SSH_KEY_PATH", keyPath)
}
  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "intimclaw-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".intimclaw", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithIntimClawHome(t *testing.T) {
	t.Setenv("INTIMCLAW_HOME", "/custom/intimclaw/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/intimclaw/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf(
			"Workspace path with INTIMCLAW_HOME = %q, want %q",
...
func TestLoadConfig_WarnsForPlaintextAPIKey(t *testing.T) {
	...
	t.Setenv("INTIMCLAW_KEY_PASSPHRASE", "test-passphrase")
	t.Setenv("INTIMCLAW_SSH_KEY_PATH", "")
...
// unchanged when INTIMCLAW_KEY_PASSPHRASE is not set.
func TestLoadConfig_NoSealWithoutPassphrase(t *testing.T) {
	...
	t.Setenv("INTIMCLAW_KEY_PASSPHRASE", "")
	t.Setenv("INTIMCLAW_SSH_KEY_PATH", "")
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when INTIMCLAW_KEY_PASSPHRASE
