...
//
// Client credentials are intentionally NOT embedded in source: they pattern-match
// GitHub push protection rules and are trivially extractable from binaries anyway.
// Configure them via environment variables instead:
//
//	ANTIGRAVITY_OAUTH_CLIENT_ID
//	ANTIGRAVITY_OAUTH_CLIENT_SECRET
//
// When unset, the returned config is empty and Antigravity login stays disabled.
// Public upstream values can be found in the OpenCode antigravity plugin sources.
...
func buildAuthorizeURL(cfg OAuthProviderConfig, pkce PKCECodes, state, redirectURI string) string {
	...
	if isGoogle {
		...
		if strings.Contains(strings.ToLower(cfg.Issuer), "auth.openai.com") {
			params.Set("originator", "intimclaw")
		}
		if cfg.Originator != "" {
