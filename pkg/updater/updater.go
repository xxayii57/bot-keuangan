...
func DownloadAndExtractRelease(releaseURL, platform, arch string) (string, error) {
	...
	// Extract
	destDir, err := os.MkdirTemp("", "intimclaw-extract-*")
	if err != nil {
		os.Remove(tmpPath)
		return "", err
	}
...
// Example: owner="sky5454" -> https://api.github.com/repos/sky5454/intimclaw/releases/latest
func GetReleaseAPIURL(owner string) string {
	return fmt.Sprintf("https://api.github.com/repos/%s/intimclaw/releases/latest", owner)
}

// GetProdReleaseAPIURL returns the production release API URL (upstream).
func GetProdReleaseAPIURL() string {
	return GetReleaseAPIURL("xxayii57")
}
...
func GetNightlyReleaseAPIURL() string {
	return GetReleaseTagAPIURL("xxayii57", "nightly")
}
