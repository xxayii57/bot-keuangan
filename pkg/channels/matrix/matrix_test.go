...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("intimclaw")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@intimclaw hello", want: true},
		{text: "hi @intimclaw:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bintimclaw\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@intimclaw:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@intimclaw:matrix.org hello", want: "hello"},
		{in: "@intimclaw, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@intimclaw:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@intimclaw:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@intimclaw:matrix.org">IntimClaw</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40intimclaw%3Amatrix.org">IntimClaw</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@intimclaw:matrix.test"), "")
	if err != nil {
