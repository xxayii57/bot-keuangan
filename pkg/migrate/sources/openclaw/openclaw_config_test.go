...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.intimclaw/workspace" {
		t.Errorf("expected workspace '~/.intimclaw/workspace', got '%s'", workspace)
	}
...
func TestConvertToIntimClaw(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToIntimClaw("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.intimclaw/workspace" {
		t.Errorf("expected workspace '~/.intimclaw/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToIntimClawWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToIntimClaw("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToIntimClawWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToIntimClawWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.intimclaw/workspace" {
		t.Errorf("expected workspace '~/.intimclaw/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
