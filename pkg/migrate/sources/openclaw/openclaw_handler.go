...
// openclaw home directory when migrating from openclaw to intimclaw.
// Default: ~/.openclaw
...
func (o *OpenclawHandler) ExecuteConfigMigration(srcConfigPath, dstConfigPath string) error {
	...
	picoCfg, warnings, err := openclawCfg.ConvertToIntimClaw(o.opts.SourceHome)
	if err != nil {
		return err
	}
...
func rewriteWorkspacePath(path string) string {
	path = strings.Replace(path, ".openclaw", ".intimclaw", 1)
	return path
}
